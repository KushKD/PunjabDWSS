...
//	alert("de_init:"+Req+":"+d__ky+":"+d__auto+":"+level2);
	alert("eeeee-------"+Req)
	add_optionbutton();
	if(d__ky==undefined || d__ky===null || d__auto===null){de_enable(false);}else{
		if(Req==undefined || Req==null || Req=='ent') {de_enable(false);}
		if(level2){
			if(Req=='ent_inquire') {de_enable(false);de_kyenable(false,d__ky,d__auto);}
			...
			if(Req=='prc') {de_enable(true);}
			if(Req=='rpt'||d__mode=='ent_add') {de_enable(true);de_autoenable(false,d__auto);}
			if(Req=='rpt'||d__mode=='ent_qrcode') {
				de_enable3(true);
				}
		}
