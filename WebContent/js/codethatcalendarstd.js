...
// http://www.codethat.com/
	var caldef1 = {
		...
		headerstyle: {
			...
			imgnextm:'next.gif',
			imgprevm:'prev.gif',
			imgnexty:'next_year.gif',
			imgprevy:'prev_year.gif' 
		},
		position: {
			absolute:true,
			x:185,y:83
	...
	function UA(){
		...
		t.win=n.indexOf('win')>=0;t.mac=n.indexOf('mac')>=0;
		t.DOM=document.getElementById?true:false;t.dynDOM=document.createElement&&document.addEventListener;t.khtml=nv.vendor=='KDE';
		...
		if(idx>=0&&!t.opera&&!t.khtml){
			...
			t.ie3down=t.vers<4;
			t.ie=t.ie4up=document.all&&document.all.item&&!t.ie3down;t.ie5up=t.ie&&t.DOM;t.ie55up=t.ie&&t.vers>=5.5;
			t.ie6up=t.ie&&t.vers>=6
		}
		t.cm=document.compatMode;t.css1cm=t.cm=='CSS1Compat';
		...
		t.hj=n.indexOf('hotjava')!=-1;t.aol=n.indexOf('aol')!=-1;
	...
	function CodeThatDateFormat(f,d,m,y){
		...
		if(CodeThatFind(document,'time')){
			...
			if(hours<0||hours>23){
				hours=hours%24;hours+=(hours<0?24:0);
			}
			...
			if(seconds<0||seconds>59){
				seconds=seconds%60;seconds+=(seconds<0?60:0);
			}
	...
	};
	{
		...
		CTc.hide=function(){
						...
						CTc.create=function(d,ctl){
							...
							d.write("<table "+(Def(this.ifr)?"align=\"center\"":"")+" cellspacing=0 cellpadding=0 "+acts+" width="+def.width+" border="+(def.border_width||0)+" bordercolor='"+(def.border_color||'#000000')+"'>");
							d.write("<tr>");
							if(i)
								...
								if(ua.opera&&w==4){
									d.write("<tr><td colspan=7 class='cal'>&nbsp;</td></tr>");
								}
								d.write("</table>");
							};
							CTc.createTime=function(d){
												d.write("<tr><td colspan=7 align=center  class='cal'>"+(ua.nn4?"<form>":"")+"<input "+(ua.nn4?"name":"id")+"='time' type=textarea value='"+CodeThatAlign(this.date.getHours())+":"+CodeThatAlign(this.date.getMinutes())+":"+CodeThatAlign(this.date.getSeconds())+"'>"+(ua.nn4?"</form>":"")+"</td></tr>");
											};
							...
							CTc.getStyle=function(){
											var c;
											if(this.i){
												var fd=CodeThatDateFormat(this.def.dtype,this.date.getDate(),this.date.getMonth(),this.date.getFullYear());
												if(this.styles[fd])
													c=this.styles[fd];
											}
										return c;
							...
							CTc.getRef=function(){
											...
											if(this.i){
												...
												ref=ref||(" href='javascript:CodeThatSetDay(\""+this.ctl+"\",\""+this.def.dtype+"\","+this.date.getDate()+","+this.date.getMonth()+","+this.date.getFullYear()+","+this.i+",\""+(Def(this.ifr)?this.ifr:"")+"\")'");
											return ref;
										};
							...
							CTc.createButtons=function(d,i){
													...
													for(var p in dh)--col;
													if(dh.imgprevy&&!i)
														d.write("<td width=14% align=left><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",3)'><IMG border=0 src='img/"+dh.imgprevy+"'</IMG></A>");
													if(dh.imgprevm&&!i)
														d.write("<td width=14% align=left><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",1)'><IMG border=0 src='img/"+dh.imgprevm+"'</IMG></A>");
													d.write("<td colspan="+col+" align=center "+this.getCss("css",dh)+">"+this.def.monthnames[this.date.getMonth()]+"&nbsp;"+this.date.getFullYear()+"</td>");
													if(dh.imgnextm&&!i)
														d.write("<td width=14% align=right><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",0)'><IMG border=0 src='img/"+dh.imgnextm+"'</IMG></A></td>");
													if(dh.imgnexty&&!i)
														d.write("<td width=14% align=right><a href='javascript:CodeThatSetMY("+this.date.getMonth()+","+this.date.getFullYear()+",2)'><IMG border=0 src='img/"+dh.imgnexty+"'</IMG></A></td>");
												};
							CTc.createCombos=function(d,l){
												...
												if(ua.nn4)
													d.write("<form>");
												d.write("<select align=left "+this.getCss("css",dh)+" "+(ua.nn4?"name":"id")+"='month' onchange='CodeThatSetMY(CodeThatFind(document,\"month\").selectedIndex,"+dh.yearrange[0]+"+CodeThatFind(document,\"year\").selectedIndex,4);'>");
												var i=0;
												for(;i<12;++i)
													d.write("<option "+(i==this.date.getMonth()?"selected='selected'":"")+" value='"+i+"'>"+this.def.monthnames[i]+"</option>");
												d.write("</select>");
												d.write("<select align=right "+this.getCss("css",dh)+" "+(ua.nn4?"name":"id")+"='year' onchange='CodeThatSetMY(CodeThatFind(document,\"month\").selectedIndex,"+dh.yearrange[0]+"+CodeThatFind(document,\"year\").selectedIndex,4);'>");
							...
							CTc.popup=function(ctl){
										...
										var w=window.open("codethatcalendar.html","",'width='+this.def.windoww+',height='+this.def.windowh+',status=no,resizable=no,top=200,left=200,dependent=yes,alwaysRaised=yes');
										w.opener=window;
										w.focus();
									};
							CTc.innerpopup=function(ctl,ifr){
												...
												var e=CodeThatFind(document,ctl);
												this.date=new Date();
											//	alert("innerpopup: "+e+":"+e.name+":"+e.value);
												if(e.value!="")
													this.parseValue(e.value);
												...
												if(ua.oldOpera){
													...
													w.src="codethatcalendar_ifr.html";
	...
	document.write(" scrolling=\"no\" style=\"filter:Alpha(Opacity=160); position:absolute;left:650;top:83;\" ");
