...
StrutsLayout.Datagrid = function Datagrid(in_property, in_styleId, in_styleClass, in_styleClass2, in_allowSelection, in_allowMultipleSelection) {
	...
	function addDatagridCell(row, index, property, styleClass, type, values) {
		...
		//alert(row+":"+ index+":"+property+":"+styleClass+":"+ type+":"+ values);
		if (type=="empty") {
			...
			if (type!="select") {
				...
				input.readOnly = true; //THIS IS TO ADD READONLY INPUT ELEMENTS WHILE ADDING NEW ROW
			}
		    ...
		    if (type=="checkbox") {	   
				...
				//alert(inputElementName+":"+ values[i].label+":"+values[i].value);
...
// add a line, PUBLIC
StrutsLayout.addDatagridLine = function(property,colValues) {				
	// Get the datagrid.
	var colValues=colValues.split('~');
	...
	for (i in datagrid.columns) {
		...
		//alert(document.getElementById(colValues[i]).value);
		var values = document.getElementById(colValues[i]).value;
...
StrutsLayout.addDatagridLineBase = function(property) {				
	// Get the datagrid.
	var datagrid = strutsLayoutDatagridData[property];
...
StrutsLayout.setDatagridLineState = function(property, state) {
	...
	console.log(document.forms[0]+":"+selectedLines);
...
//set the state of the selected lines.
StrutsLayout.setDatagridLineStateNew = function(property, state) {
	// Get the datagrid.
	var datagrid = strutsLayoutDatagridData[property];
