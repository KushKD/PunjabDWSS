...
<script type="text/javascript">
	function returnBeneficiarys() {
		...
		$('#verifyPaymentEntry1 > tbody  > tr').each(function() {
			var row = $(this).html();
			row2.push($(this).closest('tr').find('td:nth-child(8)').text());
			validationRequestId = row2.join();
		});
		document.getElementById('validationRequestId').value = validationRequestId;

		document.verifyPaymentForm.action = "verifyPaymentAction.do?method=back&d__mode="
			+ d__mode + "&menuId=SNT012&request="+validationRequestId;
	document.verifyPaymentForm.submit();
	}
	...
	var forwrdBeneficiary = {
		forwrdBeneficiarys : function() {
			var validationRequestId;
			var row2 = [];
			$('#verifyPaymentEntry1 > tbody  > tr').each(
					function() {
						var row = $(this).html();
			...
			document.verifyPaymentForm.action = "verifyPaymentAction.do?method=forward&d__mode="
					+ d__mode + "&menuId=SNT012&request="+validationRequestId;
			document.verifyPaymentForm.submit();
...
<body>
	...
	<html:form action="verifyPaymentAction" method="post"
		styleId="verifyPaymentForm">
		<div id="modalContainer"></div>
